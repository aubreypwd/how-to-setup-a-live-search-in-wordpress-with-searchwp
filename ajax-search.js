...
/*
 * First let's make sure we don't create a window object called ajaxSearch if it already exists.
 *
 * We're going to put all the stuff our JavaScript does on window.ajaxSearch so can can access it later if we need to.
 */
if ( ! window.hasOwnProperty( 'ajaxSearch' ) ) {

	/*
	 * Yup, there's that assignment I was talking about.
	 *
	 * But, it's accepting $ (which is jQuery in slang).
	 */
	window.ajaxSearch = ( function( $ ) {
		...
		// This uses jQuery to detect when the page is loaded and we can start modifying the webpage.
